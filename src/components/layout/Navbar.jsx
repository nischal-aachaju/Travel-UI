...
const Navbar = () => {
  return (
    <nav className="w-full h-20 flex items-center px-16">
      <div className="cursor-pointer font-bold text-black w-2/10 text-3xl h-full flex items-center justify-start ">
        WANDER
      </div>
      ...
      <ul className="h-full w-4/10 flex justify-evenly items-center ">
        {navItems.map((item, index) => {
          return (
            <li
              key={index}
              className="text-black/70 text-lg font-semibold hover:text-black transition"
            >
              <a
                className="
         relative
